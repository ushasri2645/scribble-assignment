  ...
  "room": {
    ...
    "participants": [
      ...
      },
      {
        "id": "uuid-2",
        "name": "Bob",
        "joinedAt": "2026-06-01T00:00:01.000Z"
      }
    ],
    "availableWords": ["rocket", "pizza", "castle", "guitar", "sunflower"],
    "roles": ["drawer", "guesser"],
...
Same room shape as `POST /rooms/:code/start`, but with updated `canvasEvents`.
...
Same room shape as `POST /rooms/:code/draw`, but `canvasEvents` is reset to an
empty list.
...
Returns the room to the lobby after an ended round.
...
### Response while active for the drawer
...
### Response while active for guessers
...
- Polling is the synchronization mechanism for canvas, guesses, scores, end
  summaries, and restart state.
- The drawer should not see the guess input in the UI during an active round,
  and guessers should not receive the secret word until the round ends.
